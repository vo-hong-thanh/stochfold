...
import simulator.openstrand.StochFold;
...
public class RunStochFold {
    ...
    public static void main(String[] args) throws Exception {
        ...
        StringBuilder outputDataFile = new StringBuilder("StochFold_");
//        
...
//        outputDataFile.append("QBeta");
        ...
        outputDataFile.append("_" + numRuns + ".txt");
        DataWriter dataWriter = new DataWriter(outputDataFile.toString());
        dataWriter.writeLine("Time\tPrediction\tFound\tRuntime\tTimeComputeStructures\tTimeSelection");
        StochFold stochFold;
        for (int i = 0; i < numRuns; i++) {
            System.out.print("run @" + i + ": ");
            stochFold = new StochFold();
            stochFold.config(sequence, startingDotBracket, targetDotBracketList, simTime, dataWriter);
            stochFold.runSim();
        }
        dataWriter.flush();
        dataWriter.close();
